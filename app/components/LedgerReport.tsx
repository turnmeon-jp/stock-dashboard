...
const VERDICT_BADGE: Record<LedgerVerdict, { label: string; cls: string }> = {
  promote_candidate: { label: "🟢 昇格候補", cls: "bg-emerald-100 text-emerald-800" },
  demote_candidate: { label: "🔴 廃止候補", cls: "bg-rose-100 text-rose-800" },
  insufficient_n: { label: "⚪ 標本不足", cls: "bg-slate-100 text-slate-500" },
  stale: { label: "🟡 発火不足", cls: "bg-amber-100 text-amber-700" },
  watch: { label: "🔵 観察中", cls: "bg-blue-100 text-blue-700" },
};
...
function VerdictBadge({ v }: { v: LedgerVerdict }) {
  ...
  return (
    <span className={`inline-block whitespace-nowrap rounded px-2 py-0.5 text-xs font-semibold ${b.cls}`}>
      {b.label}
    </span>
  );
}
      ...
      <table className="w-full text-xs">
        <thead>
          <tr className="bg-slate-100 text-slate-500 text-left">
            ...
            <th className="px-2 py-1.5 font-medium text-right whitespace-nowrap">n</th>
            <th className="px-2 py-1.5 font-medium text-right whitespace-nowrap">勝率</th>
            <th className="px-2 py-1.5 font-medium text-right whitespace-nowrap">中央値超過</th>
            <th className="px-2 py-1.5 font-medium text-right whitespace-nowrap">平均超過</th>
            <th className="px-2 py-1.5 font-medium text-right whitespace-nowrap">上位10%平均</th>
          </tr>
        </thead>
        <tbody>
          {horizons.map((h) => {
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-100 text-slate-600 text-left">
              ...
              <th className="px-3 py-2 font-medium whitespace-nowrap">判定</th>
              <th className="hidden sm:table-cell px-3 py-2 font-medium text-right whitespace-nowrap">n</th>
              <th className="hidden sm:table-cell px-3 py-2 font-medium text-right whitespace-nowrap">中央値超過(20d)</th>
              <th className="hidden sm:table-cell px-3 py-2 font-medium text-right whitespace-nowrap">勝率(20d)</th>
            </tr>
          </thead>
          <tbody>
